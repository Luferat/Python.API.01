...
def get_all_items():
    ...
    try:  # Tenta executar os comandos do bloco.
    ...
    except sqlite3.Error as error:  # Se ocorrer exceção ao processar banco de dados.
        return {"error": f"Erro ao acessar o banco de dados: {str(error)}"}

    except Exception as error:  # Se ocorrer qualquer outra exceção.
...
def get_one_item(id):
    ...
    except sqlite3.Error as error:  # Se ocorreu erro ao processar banco de dados.
        return {"error": f"Erro ao acessar o banco de dados: {str(error)}"}
